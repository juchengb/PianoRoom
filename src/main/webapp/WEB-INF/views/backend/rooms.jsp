	            ...
	            <div class="rooms-area w-100 d-flex flex-wrap overflow-auto">
					...
					<c:forEach items="${rooms}" var="room">
		                <div id="room${room.id}" class="rooms-row m-2 p-3">
		                    ...
		                    <div><span>校區：</span>${room.dist}</div>
		                    <div><span>類型：</span>${room.type}</div>
		                    <div><span>經度：</span>${room.latitude}</div>
		                    <div><span>緯度：</span>${room.longitude}</div>
		                    <div><span>營業時間：</span>
		                    	<span class="text-black">星期一 ${room.businessHour[0].openingTime} - ${room.businessHour[0].closingTime}...</span>
		                    </div>
		                </div>
					</c:forEach>
				</div>
